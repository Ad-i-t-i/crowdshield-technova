    ...
    <div className="clay-card-puffy p-5 flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <div>
          ...
          <p className="text-xs text-slate-400">Core USP Analytics Engine</p>
        </div>
      ...
      {/* ⚠️ 10-Minute Warning Banner */}
      {threatLevel >= 75 && (
        <div className="rounded-2xl border-2 border-red-500 bg-red-950/80 p-4 shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-4 animate-pulse">
          ...
          <div className="flex items-center gap-3 bg-[#0c101c] px-4 py-2 rounded-2xl border border-red-500/50">
            ...
            <button
              ...
              className="clay-btn-danger px-4 py-2 text-xs font-black"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
        {/* Stampede Risk KPI */}
        <div className={`p-4 rounded-2xl border ${threatLevel >= 75 ? 'bg-red-950/50 border-red-500/60 shadow-lg' : threatLevel >= 45 ? 'bg-amber-950/40 border-amber-500/50' : 'bg-[#0c101c] border-slate-800'}`}>
          <div className="flex items-center justify-between text-xs text-slate-400 mb-1 font-bold">
            <span>Stampede Likelihood (USP)</span>
            <TrendingUp className={`w-4 h-4 ${threatLevel >= 75 ? 'text-red-400' : 'text-emerald-400'}`} />
          </div>
          <div className="flex items-baseline gap-2">
            <span className={`text-3xl font-black font-heading ${threatLevel >= 75 ? 'text-red-400' : threatLevel >= 45 ? 'text-amber-400' : 'text-emerald-400'}`}>
              {threatLevel}%
            </span>
        ...
        {/* Density KPI */}
        <div className="p-4 rounded-2xl bg-[#0c101c] border border-slate-800">
          <div className="flex items-center justify-between text-xs text-slate-400 mb-1 font-bold">
            <span>Crowd Density</span>
            <Zap className="w-4 h-4 text-blue-400" />
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-black font-heading text-blue-400">
        ...
        <div className="p-4 rounded-2xl bg-[#0c101c] border border-slate-800">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-black font-heading text-sky-400">
      ...
      {/* Chart */}
      <div className="w-full bg-[#070a14] p-4 rounded-2xl border border-slate-800 flex flex-col gap-3">
        <div className="flex items-center justify-between text-xs text-slate-400 font-mono">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-blue-500" /> Density</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-sky-400" /> Speed</span>
          </div>
        </div>
        ...
        <div className="h-52 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Tooltip 
                contentStyle={{ backgroundColor: '#0c101c', borderColor: '#1e293b', borderRadius: '12px', fontSize: '12px' }}
                labelStyle={{ color: '#94a3b8' }}
              />
              <Area type="monotone" dataKey="risk" stroke="#ef4444" strokeWidth={2.5} fillOpacity={0.2} fill="#ef4444" name="Stampede Risk %" />
              <Area type="monotone" dataKey="density" stroke="#2563eb" strokeWidth={2} fillOpacity={0.15} fill="#2563eb" name="Density (ppl/m²)" />
              <Area type="monotone" dataKey="speed" stroke="#38bdf8" strokeWidth={2} fillOpacity={0} fill="none" name="Speed (m/s)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
