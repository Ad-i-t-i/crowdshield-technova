...
import { ShieldCheck, ShieldAlert, AlertTriangle, Users, Sun, LayoutDashboard, BarChart3, ShieldCheck as ShieldIcon, Camera, Smartphone } from 'lucide-react';
      ...
      <div className="clay-card-puffy p-4 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono">
              {time.toLocaleDateString()} • {time.toLocaleTimeString()} (LIVE SYNCHRONIZED)
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`text-xs font-bold py-1.5 px-3 rounded-2xl transition-all ${
              simpleView 
                ? 'bg-gradient-to-r from-[#C3B1E1] to-[#9B7FD4] text-[#0D1B2A] shadow shadow-[#C3B1E1]/30 font-black' 
                : 'bg-[#0D1B2A] text-slate-300 hover:bg-slate-800 border border-slate-800'
            }`}
          >
            {simpleView ? '✨ Simple View' : '⚙️ Detailed View'}
          </button>

          <button
            onClick={onResetSimulation}
            className="clay-btn-blue text-xs font-bold py-1.5 px-3"
          >
            Reset
          </button>
        </div>
