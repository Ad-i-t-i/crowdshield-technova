    ...
    const render = () => {
      ...
      faces.forEach((face) => {
        ...
        if (privacyShieldActive) {
          ctx.fillStyle = 'rgba(168, 216, 234, 0.85)';
          ctx.fillRect(face.x - 2, face.y - 2, face.size + 4, face.size * 1.3);

          ctx.strokeStyle = '#AEDCC0';
          ctx.lineWidth = 1.5;
    ...
    <div className="clay-card-puffy p-5 flex flex-col gap-3.5">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <div className="flex items-center gap-2">
          <Camera className="w-4 h-4 text-[#A8D8EA]" />
        ...
        <select
          ...
          className="bg-[#0D1B2A] border border-slate-800 text-slate-200 text-xs px-2.5 py-1 rounded-xl focus:outline-none cursor-pointer"
        >
      ...
      <div className="relative rounded-2xl overflow-hidden bg-[#0D1B2A] border border-slate-800 shadow-xl">
        <canvas ref={canvasRef} className="w-full block" style={{ height: '190px' }} />

        <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-[#0D1B2A] border border-slate-800 text-[10px] font-mono">
        ...
        <div className="absolute top-2.5 right-2.5">
          {privacyShieldActive ? (
            <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-[#AEDCC0]/20 border border-[#AEDCC0] text-[#AEDCC0] text-[10px] font-black font-mono">
              <Lock className="w-3 h-3 text-[#AEDCC0]" />
              <span>PRIVACY SHIELD ACTIVE</span>
            </div>
          ) : (
            <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-[#FF8B94]/20 border border-[#FF8B94] text-[#FF8B94] text-[10px] font-black font-mono">
              <Eye className="w-3 h-3 text-[#FF8B94]" />
      ...
      <div className="flex items-center justify-between p-3 rounded-2xl bg-[#0D1B2A] border border-slate-800">
        <div className="flex items-center gap-2">
          <ShieldCheck className={`w-4 h-4 ${privacyShieldActive ? 'text-[#AEDCC0]' : 'text-slate-500'}`} />
        ...
        <button
          ...
          className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
            privacyShieldActive ? 'bg-[#AEDCC0]' : 'bg-slate-700'
          }`}
        >
          <span
            className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-[#0D1B2A] shadow ring-0 transition duration-200 ease-in-out ${
